...
# ui =============================================================
ui <- navbarPage(title = "AirSensEUR v0.15", id = "ASE", theme = shinytheme("cerulean"), selected = "SelectASE",
...
server <- function(input, output, session) {
    ...
    list.gas.sensors       <- reactive({Config$all[["sens2ref"]]$gas.sensor[ !is.na(Config$all[["sens2ref"]]$gas.sensor)  & Config$all[["sens2ref"]]$gas.sensor  != "" & Config$all[["sens2ref"]]$name.sensor %in% Config$all$sens2ref.shield$name.sensor]})
    list.name.sensors      <- reactive({Config$all[["sens2ref"]]$name.sensor[!is.na(Config$all[["sens2ref"]]$name.sensor) & Config$all[["sens2ref"]]$name.sensor != "" & Config$all[["sens2ref"]]$name.sensor %in% Config$all$sens2ref.shield$name.sensor]})
    list.gas.reference2use <- reactive({
        # return a vector with the names of gas reference using the file ASE.cfg
        # only return the list of Reference gas if they are  included into the RefData file (names(Ref$DATA))

        # test if there are Config$all[["sens2ref"]]$gas.reference2use in common with names(Ref$DATA
        # if not send a message to add reference data and the App will crash
        Config$all[["sens2ref"]]$gas.reference2use[!is.na(Config$all[["sens2ref"]]$gas.reference2use) &
                                            Config$all[["sens2ref"]]$gas.reference2use %in% names(Ref$DATA)]
    })
    ...
    observeEvent(input$Selected, {
        ...
        if (input$Select > 0) {
            ...
            if (file.exists(General.file())) {
                if (extension(General.file()) == ".csv") {
                    ...
                    if (!all("" %in% c(Config$all$Server$Influx.TZ, Config$all$Server$SOS.TZ))) {
                        ...
                        } else data.table::set(DF$General, j = "date", value =  ymd_hms(DF$General[["date"]], tz = Config$all$Server$SOS.TZ))
                    } else data.table::set(DF$General, j = "date", value =  ymd_hms(DF$General[["date"]], tz = "UTC"))
            ...
            Covariates        <- reactive({c(paste0("Out.",list.gas.reference2use()), INFLUX()[[2]], "Absolute_humidity", "Td_deficit","date", paste0(list.gas.sensors(),"_modelled"), paste0(list.name.sensors(),"_volt"), paste0("Out.",list.gas.sensors())) })
            Covariates.Model  <- reactive({c(paste0("Out.",list.gas.reference2use()), INFLUX()[[2]], "Absolute_humidity", "Td_deficit","date", paste0(list.gas.sensors(),"_modelled"), paste0(list.name.sensors(),"_volt"), paste0("Out.",list.gas.sensors())) })
            ...
            output$Influx.Content    <- renderPrint({
                ...
                if (!is.null(Influx$DATA)) return(Influx$DATA) else return("[shiny, Influx.Content] INFO, file InfluxData.Rdata does not exist")
            })
            output$SOS.Content    <- renderPrint({

                # Checking if the Rdata exist
                if (!is.null(Sos$DATA)) return(Sos$DATA) else return("[shiny, SOS.Content] INFO, file SOSData.Rdata does not exist")
            })
            output$Ref.content <- renderPrint({

                # Checking if the Rdata exist
                if (!is.null(Ref$DATA)) return(Ref$DATA) else return("[shiny, Ref.content] INFO, file RefData.Rdata does not exist")
            })
            output$General.Content <- renderPrint({

                # Checking if the Rdata exist
                if (!is.null(DF$General)) return(DF$General) else return("[Shiny,General.Content] INFO, file General.Rdata does not exist")
            })
                                                          ...
                                                          tabPanel(
                                                              ...
                                                              div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                                                  numericInput(inputId  = paste0("Warming",i)     ,
                                                                               ...
                                                                               min = 0  ,
                                                                               max = 1440, step = 1  ,
                                                                               value = Config$all[["sens2ref"]]$hoursWarming[i.sensors()][i])
                                                              ),
                                                              hr(),
                                                              ...
                                                              sliderInput(inputId   = paste0("Temperature",i) ,
                                                                          ...
                                                                          max = 60  ,
                                                                          step = 0.5,
                                                                          value = c(Config$all[["sens2ref"]]$temp.thres.min[i.sensors()][i],
                                                                                    Config$all[["sens2ref"]]$temp.thres.max[i.sensors()][i])
                                                              ),
                                                              sliderInput(inputId   = paste0("Humidity",i)    ,
                                                                          ...
                                                                          max = 100 ,
                                                                          step = 0.5  ,
                                                                          value = c(Config$all[["sens2ref"]]$rh.thres.min[i.sensors()][i]  ,
                                                                                    Config$all[["sens2ref"]]$rh.thres.max[i.sensors()][i])
                                                              ),
                                                              hr(),
                                                              div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                                                  checkboxInput(inputId = paste0("Sens.Inval.Out", i),
                                                                                label = "Discard Invalid data"  ,
                                                                                value = Set$Time$Sens.Inval.Out[i.sensors()][i])
                                                              ),
                                                              div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                                              ...
                                                              div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                                                  checkboxInput(inputId = paste0("Sens.rm.Out", i),
                                                                                ...
                                                                                value = Config$all[["sens2ref"]]$Sens.rm.Out[i.sensors()][i]
                                                                  )
                                                              ),
                                                              div(style = "display: inline-block;vertical-align:top; width: 49%;",
                            ...
                            tabPanel(title =  "Reference", icon = icon("calculator"),
                                     do.call(tabsetPanel,
                                             c(id='Filtering.References',
                                               lapply(seq_along(Config$all[["sens2ref"]]$gas.reference),
                                                      function(i) {
                                                          tabPanel(
                                                              title =Config$all[["sens2ref"]]$gas.reference[i],
                                                              ...
                                                              div(style = "display: inline-block;vertical-align:top; width: 77%;",
                                                                  dateRangeInput(inputId = paste0("Out.Ref.Date", i),
                                                                                 ...
                                                                                 format  = "yyyy-mm-dd",
                                                                                 start   = Set$Time[["Out.Ref.IN"]][i],
                                                                                 end     = as.Date(Set$Time[["Out.Ref.END"]][i]),
                                                                                 weekstart = 1,
                                     ...
                                     tabPanel(
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             selectInput(  inputId  = paste0("Sens.raw.unit", i),
                                                           ...
                                                           selected = Config$all[["sens2ref"]]$Sens.raw.unit[i.sensors()][i])),
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             selectInput(  inputId  = paste0("Sens.unit", i),
                                                           ...
                                                           selected = Config$all[["sens2ref"]]$Sens.unit[i.sensors()][i])),
                                         ...
                                         selectInput(  inputId  = paste0("Sens", i),
                                                       ...
                                                       choices  = Covariates(),
                                                       selected = as.vector(Config$all[["CovPlot"]][[list.name.sensors()[i]]]$Effects),
                                                       multiple = TRUE),
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 64%;",
                                             radioButtons( inputId  = paste0("Cal.Line", i),
                                                           ...
                                                           selected = Config$all[["sens2ref"]]$Cal.Line[i.sensors()][i])),
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             selectInput(  inputId  = paste0("Calibration", i),
                                                           ...
                                                           selected = Config$all[["sens2ref"]]$mod.eta.model.type[i.sensors()][i])),
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             selectInput(  inputId  = paste0("CovMod", i),
                                                           ...
                                                           selected = as.vector(Config$all[["CovMod"]][[list.name.sensors()[i]]]$Effects),
                                                           multiple = TRUE)),
                                         div(style = "display: inline-block;vertical-align:top; width: 94%;",
                                             selectInput(  inputId  = paste0("Cal", i),
                                                           ...
                                                           choices  = substr(list.files(path = file.path(DisqueFieldtestDir(),"Models"),
                                                                                        pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"__",list.name.sensors()[i],"__*"))),
                                                                             start = nchar(paste0(Config$all[["Server"]]$AirsensEur.name,"__",list.name.sensors()[i],"__")) + 1,
                                                                             stop  = nchar(list.files(path = file.path(DisqueFieldtestDir(),"Models"),
                                                                                                      pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"__",list.name.sensors()[i],"__*"))))),
                                                           selected = Config$all[["sens2ref"]]$Cal.func[i.sensors()][i])),
                                         div(style = "display: inline-block;vertical-align:top; width: 5%;",
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 47%;",
                                             numericInput(inputId = paste0("Slope", i),
                                                          ...
                                                          value  = Config$all[["sens2ref"]]$Slope[i.sensors()][i], step = 0.000001)),
                                         div(style = "display: inline-block;vertical-align:top; width: 1%;",HTML("<br>")),
                                         div(style = "display: inline-block;vertical-align:top; width: 47%;",
                                             numericInput(inputId = paste0("Intercept", i),
                                                          label = "Intercept for calibration"            ,
                                                          value = Config$all[["sens2ref"]]$Intercept[i.sensors()][i], step = 0.0001)),

                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                             checkboxInput(inputId = paste0("Neg.mod", i),
                                                           label   = "Discard negative predicted data?",
                                                           value   = Config$all[["sens2ref"]]$Neg.mod[i.sensors()][i])),
                                         div(style = "display: inline-block;vertical-align:top; width: 49%;",
                                     ...
                                     tabPanel(
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 77%;",
                                             dateRangeInput(inputId = paste0("Valid", i),
                                                            ...
                                                            format  = "yyyy-mm-dd",
                                                            start   = Set$Time[["Valid.IN"]][i.sensors()[i]],
                                                            end     = as.Date(Set$Time[["Valid.END"]][i.sensors()][i]),
                                                            weekstart = 1,
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 77%;",
                                             dateRangeInput(inputId = paste0("Out.Sens.Date", i),
                                                            ...
                                                            format  = "yyyy-mm-dd",
                                                            start   = Set$Time$Out.Sens.IN[i.sensors()][i],
                                                            end     = as.Date(Set$Time$Out.Sens.END[i.sensors()][i]),
                                                            weekstart = 1,
                                                            min = Set$Time[["Valid.IN"]][i.sensors()[i]],
                                                            max = Set$Time[["Valid.END"]][i.sensors()[i]]
                                             )
                                         ...
                                         div(style = "display: inline-block;vertical-align:top; width: 6%;",
                                             bsButton(inputId = paste0("DateCOVCal", i), label = "Cal", icon = NULL, style = "default",
                                                      size = "extra-small", type = "action", block = FALSE, disabled = FALSE,
                                                      value = FALSE)
                                         ),
                                         div(style = "display: inline-block;vertical-align:top; width: 4%;",
            ...
            if (input$asc.File != Config$all[["Server"]]$asc.File) {
                ...
                Delete.General <- function(value) {
                    ...
                    if (value == TRUE) {
                        ...
                        # updateTextInput(session, inputId = "Selected", value = input$Selected)
                        #shinyjs::click(id = "Merge")
                    }
                }
    ...
    INFLUX <- eventReactive({
        ...
        # updating Input$DATA
        if (!is.null(A[[1]])) Influx$DATA <<- A[[1]]
        return(A)
    })
    ...
    REFDATA <- eventReactive({
        ...
        if ((!is.null(input$Down_Ref) && input$Down_Ref > 0) ||
            ...
            # updating Ref$DATA
            if (!is.null(C[[1]])) Ref$DATA        <<- C[[1]]
            # updating Download$Sensor
            if (!is.null(C[[3]])) Download$Sensor <<- C[[3]]
            return(C)
    ...
    observeEvent(input$Merge,{
        ...
        observeEvent({
            unlist(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference), function(i) input[[paste0("left2Out.Ref.Date", i)]]))
        },{
            # index 1,2,3,4, 5, 6 of the reference gas selected in uiFiltering corresponding of rows of ASE_name.cfg
            Ref.Index <-  match(x = input$Filtering.References, table = Config$all[["sens2ref"]]$gas.reference)
            # Corresponding index for Valid
            # Corresponding index for input$Valid
            Sens.Index <-  match(x = Config$all[["sens2ref"]]$gas.sensor[Ref.Index], table = list.gas.sensors())
        ...
        ignoreInit = TRUE)
        observeEvent({
            unlist(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference), function(i) input[[paste0("left1Out.Ref.Date", i)]]))
        }, {
            # index 1,2,3,4, 5, 6 of the reference gas selected in uiFiltering corresponding of rows of ASE_name.cfg
            Ref.Index  <-  match(x = input$Filtering.References, table = Config$all[["sens2ref"]]$gas.reference)
            # Corresponding index for input$Valid
        ...
        observeEvent({
            unlist(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference), function(i) input[[paste0("right1Out.Ref.Date", i)]]))
        }, {
            # index 1,2,3,4, 5, 6 of the reference gas selected in uiFiltering corresponding of rows of ASE_name.cfg
            Ref.Index <-  match(x = input$Filtering.References, table = Config$all[["sens2ref"]]$gas.reference)
            # Corresponding index for input$Valid
        ...
        observeEvent({
            unlist(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference), function(i) input[[paste0("right2Out.Ref.Date", i)]]))
        }, {
            # index 1,2,3,4, 5, 6 of the reference gas selected in uiFiltering corresponding of rows of ASE_name.cfg
            Ref.Index <-  match(x = input$Filtering.References, table = Config$all[["sens2ref"]]$gas.reference)
            # Corresponding index for input$Valid
        ...
        observeEvent(input$Delete.Model, {
            ...
            if (input$Delete.Model) {
                ...
                Cal  <- paste0(Config$all[["Server"]]$AirsensEur.name,"__",input$Calib.Sensors,"__",input[[paste0("Cal",k)]])
                ...
                if (!is.null(Cal)) {
                    ...
                    choices <- substr(list.files(path = file.path(DisqueFieldtestDir(),"Models"),
                                                 pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"*",input$Calib.Sensors,"*"))),
                                      start = nchar(paste0(Config$all[["Server"]]$AirsensEur.name,"__",input$Calib.Sensors,"__")) + 1,
                                      stop  = nchar(list.files(path = file.path(DisqueFieldtestDir(),"Models"),
                                                               pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"*",input$Calib.Sensors,"*"))))
                    )
                        ...
                        if (input$Down.Ref && (
                            ...
                            if (input$Down.SOS && input$Down.Influx) {
                                ...
                                if (!file.exists(General.file()) ) {
                                    ...
                                    # This is not necessary anymore since the file is loaded when selecting the AirSensEUR and put into DF$General
                                    DF$General <<- data.table::fread(General.file()) # , na.strings=getOption("","NA")
                                    ...
                                    # let's use identical
                                    if (!"data.table" %in% class(DF$General)) DF$General <- data.table(DF$General, key = "date")
                                    Common.cols <- intersect(names(D),names(DF$General))
                                    if (!isTRUE(all.equal(D[,..Common.cols], DF$General[,..Common.cols]))) {
                                        save.General.df <- TRUE
                                        DF$General <- D
                                    }
                                }
        ...
        Outliers_Sensor        <- reactive({
            # Create a Progress object
            progress <- shiny::Progress$new()
            progress$set(message = "[Shiny, Outliers_Sensor()] INFO, Reading sensor config file", value = 0.5)
            # Make sure it closes when we exit this reactive, even if there's an error
            on.exit(progress$close())
        ...
        Outliers_Ref           <- reactive({
            # Create a Progress object
            progress <- shiny::Progress$new()
            progress$set(message = "[Shiny, Outliers_Ref] INFO, Reading Referance data config file", value = 0.5)
            # Make sure it closes when we exit this reactive, even if there's an error
            on.exit(progress$close())
            ...
            Reference.Outliers <- data.frame(
                name.gas           = Config$all[["sens2ref"]]$name.gas,
                name.sensor        = as.character(Config$all[["sens2ref"]]$name.sensor),
                remove.neg         = as.logical(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("rm.neg",i)]])),
                ref.unitgas        = as.character(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.unit",i)]])),
                Ref.rm.Out         = as.logical(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.rm.Out",i)]])),
                Ref.window         = as.integer(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.window",i)]])),
                Ref.threshold      = as.numeric(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.threshold",i)]])),
                Ref.Ymin           = as.numeric(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.Ymin",i)]])),
                Ref.Ymax           = as.numeric(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.Ymax",i)]])),
                Ref.ThresholdMin   = as.numeric(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.ThresholdMin",i)]])),
                Ref.iterations     = as.integer(  sapply(seq_along(Config$all[["sens2ref"]]$gas.reference2use), function(i) input[[paste0("Ref.iterations",i)]])),
                stringsAsFactors = FALSE)
        ...
        Calib_data               <- reactive({
            ...
            name.gas.name.sensor   <- data.frame(
                name.gas           = Config$all[["sens2ref"]]$name.gas,
                name.sensor        = as.character(Config$all[["sens2ref"]]$name.sensor),
                gas.reference      = Config$all[["sens2ref"]]$gas.reference,
                gas.reference2use  = Config$all[["sens2ref"]]$gas.reference2use,
                gas.sensor         = Config$all[["sens2ref"]]$gas.sensor,
                # used in in list of sensors not be be converted as sensors in Shield()
                stringsAsFactors = FALSE)
            ...
            Sensors.Outliers       <- data.frame(
                ...
                ubsRM              = as.numeric(  sapply(seq_along(list.gas.sensors()), function(i) input[[paste0("ubsRM",i)]])),
                ubss               = as.numeric(  sapply(seq_along(list.gas.sensors()), function(i) input[[paste0("ubss",i)]])),
        ...
        observeEvent(Warm$Forced, {
            ...
            if (Warm$Forced) {
                ...
                if (!is.null(DF$General[,"boardTimeStamp"]) ) { # use to be class(DF$General) == "data.frame"
                    ...
                    data.table::set(DF$General, i = NULL, j = "boardTimeStamp", value = na.locf(DF$General[["boardTimeStamp"]], na.rm = FALSE, fromLast = FALSE))
                    ...
                    # This is for SOS
                    ind <- apply(DF$General[, list.gas.sensors(), with = FALSE  ], 1, function(i) !all(is.na(i)))
                ...
                for (n in seq_along(list.gas.sensors())) {
                    ...
                    indfull <- indfull[indfull <= length(DF$General[[list.gas.sensors()[n]]])]
        ...
        Plot.Warming   <- reactive({
            ...
            unlist(lapply(list.gas.sensors()[!is.na(list.name.sensors())], function(i) {
                cat(paste0("[shiny, Plot.Warming()] INFO, sensor ",
                           Config$all[["sens2ref"]]$name.sensor[match(x = i, table = Config$all[["sens2ref"]]$gas.sensor)],
        ...
        observeEvent(TRh$Forced, {
            ...
            if (TRh$Forced) {
                ...
                for (l in list.gas.sensors()) {
                    ...
                    ind <- (DF$General[, index.temp, with = FALSE]   < input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][1]  |
                                DF$General[, index.temp, with = FALSE]   > input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][2]) |
                        (DF$General[, index.rh, with = FALSE]     < input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][1]  |
                             DF$General[, index.rh, with = FALSE]     > input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][2])

                    # Global index of temperature/humidity exceeding thresholds
                    T.min  <- DF$General[, index.temp, with = FALSE] < input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][1]
                    T.max  <- DF$General[, index.temp, with = FALSE] > input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][2]
                    Rh.min <- DF$General[, index.rh, with = FALSE]   < input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][1]
                    Rh.max <- DF$General[, index.rh, with = FALSE]   > input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][2]

                    # if (exists("return.ind.TRh")) {
                    return.ind.TRh[[Config$all[["sens2ref"]][[which(Config$all[["sens2ref"]]$gas.sensor == l),"name.sensor"]]]]    <- which(ind)
                    return.ind.T.min[[ paste0(na.omit(Config$all[["sens2ref"]][Config$all[["sens2ref"]]$gas.sensor == l,"name.sensor"]),"__Temp. < ",input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][1])]] <- which(T.min)
                    return.ind.T.max[[ paste0(na.omit(Config$all[["sens2ref"]][Config$all[["sens2ref"]]$gas.sensor == l,"name.sensor"]),"__Temp. > ",input[[paste0("Temperature",match(x = l, table = list.gas.sensors()))]][2])]] <- which(T.max)
                    return.ind.Rh.min[[paste0(na.omit(Config$all[["sens2ref"]][Config$all[["sens2ref"]]$gas.sensor == l,"name.sensor"]),"__RH < "   ,input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][1])]] <- which(Rh.min)
                    return.ind.Rh.max[[paste0(na.omit(Config$all[["sens2ref"]][Config$all[["sens2ref"]]$gas.sensor == l,"name.sensor"]),"__RH > "   ,input[[paste0("Humidity"   ,match(x = l, table = list.gas.sensors()))]][2])]] <- which(Rh.max)
                }
        ...
        Plot.Neg.values      <- reactive({
            ...
            j <- match(x = CalSet()$name.reference , table = Config$all[["sens2ref"]]$gas.reference2use)
        ...
        Plot.Invalid.Sens      <- reactive({
            ...
            # depends: Config$all[["sens2ref"]]$gas.sensor, input$Sensors, Config$all[["sens2ref"]]$name.sensor, i.sensors(),
                ...
                for (i in list.gas.sensors()) {
                    ...
                    if (i %in% names(DF$General)) {
                        ...
                        if (!is.null(ind.warm$out[i][[1]])) {
                            ...
                            i.Rows <- as.integer(ind.warm$out[i][[1]])
                        ...
                        if (!is.null(ind.TRh$out$ind.TR[Sensor.i][[1]]) && length(ind.TRh$out$ind.TR[Sensor.i][[1]]) > 0) {
                        ...
                        if (!is.null(ind.Invalid$out[[2]][Sensor.i][[1]]) && length(ind.Invalid$out[[2]][Sensor.i][[1]]) > 0) {
                            Vector.columns <- paste0(c("Out.", "Out.Invalid." , "Out.Warm.TRh.Inv."),i)
                            set(DF$General,i = which(DF$General$date %in% ind.Invalid$out[[2]][Sensor.i][[1]]), j = Vector.columns,
                                value = rep(list(rep(NA, times = length(which(DF$General$date %in% ind.Invalid$out[[2]][Sensor.i][[1]])))), times = length(Vector.columns)))
                        }
                        ...
                        set(DF$General,i = NULL, j = paste0("Out.",i,".",1:input[[paste0("Sens.iterations", k)]]),
                        ...
                        repeat (
                            if (any(grepl(pattern = paste0("Out.",i,".", j + 1)      , x = names(DF$General)))) {
                                set(DF$General,i = NULL, j = paste0("Out.",i,".",j + 1), value = NULL)
                                j <- j + 1
        ...
        priority = 151)
        observeEvent({
            unlist(sapply(seq_along(Config$all[["sens2ref"]]$gas.reference), function(i) input[[paste0("Apply.R.Out", i)]]))
            Neg$Forced
        },{
        ...
        observeEvent(Outliers.Ref$Forced,{
            ...
            if (Outliers.Ref$Forced) {
                ...
                for (i in list.gas.reference2use()  ) {
                    ...
                    set(DF$General,i = NULL, j = Vector.columns, value = rep(list(DF$General[[i]]), times = length(Vector.columns)))

                    # discarding negative values if needed
                    # number index of reference pollutant in the list of references
                    k <- match(x = i, table = Config$all[["sens2ref"]]$gas.reference2use)
                    ...
                    set(DF$General,i = NULL, j = paste0("Out.",i,".",1:input[[paste0("Ref.iterations", k)]]),
        ...
        Plot.Sens.Outliers   <- reactive({
            ...
            # depends: Config$all[["sens2ref"]]$gas.sensor, input$Sensors, Config$all[["sens2ref"]]$name.sensor, i.sensors(), DF$General, input[[paste0("Out.Sens.Date",k)]],DisqueFieldtestDir(),
            #
            ...
            if (input[[paste0("Sens.rm.Out",CalSet()$k)]]) {
                ...
                cat(paste0("[Shiny, Plot.Sens.Outliers()], INFO, discarding of outliers not requested for ", Config$all[["sens2ref"]]$name.sensor[CalSet()$k], " in the Graphical User Interface "), sep = "\n")
                plot(1,1,col = "white", xlab = "", ylab = "", xaxt = "n", yaxt = "n", cex = 1.2)
                text(1,1,paste0("[Shiny, Plot.Sens.Outliers()], INFO, discarding of outliers not requested for ", Config$all[["sens2ref"]]$name.sensor[CalSet()$k], " in the Graphical User Interface "))
            }
            cat("-----------------------------------------------------------------------------------\n")
            cat("\n")
        ...
        Plot.Ref.Outliers    <- reactive({
            ...
            # index 1,2,3,4, 5, 6 of the reference gas selected in uiFiltering corresponding of rows of ASE_name.cfg
            k <-  match(x = input$Filtering.References, table = Config$all[["sens2ref"]]$gas.reference)
            # Selecting gas.reference2use corresponding to input$Filtering.References selected
            i <-  Config$all[["sens2ref"]]$gas.reference2use[k]
        ...
        StatFiltered               <- reactive({
            ...
            StatFiltered   <- data.frame(
                ...
                gas.reference2use   = Config$all[["sens2ref"]]$gas.reference2use[i.sensors()],
        ...
        output$ts_Cov_dygraphs <- renderUI({
            ...
            #a
            if (all(is.na(DF$General[, .SD, .SDcols = input[[paste0("Sens",CalSet()$k)]]]))) {
                ...
                #     dplyr::filter(date >= input[[paste0("Date",CalSet()$k)]][1] & date <= input[[paste0("Date",CalSet()$k)]][2] + 1) # add one day because input[[paste0("Date",CalSet()$k)]] without hours
        ...
        Plot.ValidCovarMatrix   <- reactive({
            ...
            if (all(is.na(DF$General[,INFLUX()[[4]]]))) {
                ...
                pairs(x = DF$General[date >= input[[paste0("Date",CalSet()$k)]][1] & date <= input[[paste0("Date",CalSet()$k)]][2] + 1, ..Relationships],
        ...
        observeEvent({
            ...
            if (!is.null(input[[paste0("Cal",CalSet()$k)]])) {
                ...
                if (input[[paste0("Cal",CalSet()$k)]] != "") {
                    ...
                    if (!input[[paste0("Apply.cal", CalSet()$k)]]) updateCheckboxInput(session, inputId = paste0("Apply.cal", CalSet()$k), label = NULL , value = TRUE)
        ...
        observeEvent({
            ...
            if (Conv$Forced || Cal$Forced) { # "Outliers.Ref$Forced |" not needed since if Outliers.Ref$Forced is TRUE then Cal$Forced is set to TRUE
                ...
                if (Conv$Forced) {
                    ...
                    data.table::set(DF$General, i = NULL, j = paste0(Shield()$name.sensor,"_volt"),
                    ...
                    x <- t(matrix(data = rep(x     = (Config$all[["sens2ref"]]$Ref - Config$all[["sens2ref"]]$RefAD),
                                             times = nrow(DF$General)), ncol = nrow(DF$General) ))[,i.sensors()]
                    # https://stackoverflow.com/questions/6819804/how-to-convert-a-matrix-to-a-list-of-column-vectors-in-r
                    data.table::set(DF$General, i = NULL, j = paste0(Shield()$name.sensor,"_DV"),
                    ...
                    if (length(No.Shield.gas.Sensors) > 0) {
                        ...
                        data.table::set(DF$General, i = NULL, j =  paste0(No.Shield.name.Sensors,"_volt"),
                                        value = lapply(seq_len(ncol(x)), function(i) x[,i]))
                        rm(x)
                    }
                        ...
                        if (CalSet()$Cal_Line == "Prediction with previous calibration") { #if a for loop is used, replace CalSet()$Cal_Line with input[[paste0("Cal.Line",k)]]
                            ...
                            if (nchar(CalSet()$Cal) != 0) { #if a for loop is used replace, CalSet()$Cal with input[[paste0("Cal",k)]]
                                ...
                                if (file.exists(name.Model.i)) { # & input[[paste0("Cal",k)]] != ""
                                    ...
                                    if (extension(name.Model.i) == ".rdata") Model.i <- load_obj(name.Model.i) else if (extension(name.Model.i) == ".rds") {
                                        ...
                                        if (file.exists(name.Model.i)) {
                                            ...
                                            Newchoices <- substr(list.files(path    = CalSet()$WDoutputMod,
                                                                            pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"__",CalSet()$name.sensor,"__*"))),
                                                                 start = nchar(paste0(Config$all[["Server"]]$AirsensEur.name,"__",CalSet()$name.sensor,"__")) + 1,
                                                                 stop  = nchar(list.files(path    = CalSet()$WDoutputMod,
                                                                                          pattern = glob2rx(paste0(Config$all[["Server"]]$AirsensEur.name,"*",CalSet()$name.sensor,"*")))))

                                            # Current model
                                            NewModel  <-  name.Model.i %>%
                                                basename(.) %>%
                                                sub(pattern = ".rds", replacement = ".rdata", x = .) %>%
                                                sub(pattern = paste0(Config$all[["Server"]]$AirsensEur.name,"__",CalSet()$name.sensor,"__"), replacement = "", x = .)
                                    ...
                                    if (Mod_type != "MultiLinear" || (Mod_type == "MultiLinear" && all(CovMod %in% names(DF$General)))) {
                                        data.table::set(DF$General, i = is.not.NA.y, j = nameGasMod,
                                                        value = list(Meas_Function(y          = DF$General[[nameGasVolt]][is.not.NA.y],
                                                                                   ...
                                                                                   Model      = Model.i,
                                                                                   Matrice    = Matrice)))
                                        #DF$General <- DF_avg(DF$General, width = as.numeric(input$UserMins))
                                    ...
                                    my_message <- paste0(paste0("[shiny, Cal$Forced] INFO, there is no calibration function for sensors: ", list.name.sensors()[k], "\n"))
                                    cat(my_message)
                                    shinyalert(
                ...
                for (i in seq_along(Config$all[["sens2ref"]]$gas.reference)) {
        ...
        CalSet     <- eventReactive({
            ...
            Config$all[["Server"]]$AirsensEur.name
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("CovMod", i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("ubsRM" , i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("ubss" , i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("Neg.mod" , i)]])
            ASE_name()
            ...
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("DateMeas", i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("DatePlotMeas", i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("DateCal", i)]])
            sapply(seq_along(list.name.sensors()), function(i) input[[paste0("DatePlotCal", i)]])
        },{
            ...
            if (!is.null(input[[paste0("Cal",k)]]) && !is.na(input[[paste0("Cal",k)]]) && is.nan(input[[paste0("Cal",k)]])) {
                ...
                if (input[[paste0("Cal",k)]] == "") NewCalSet <- "" else {
                    ...
                    NewCalSet <- Models[grep(pattern = paste0("_",strsplit(input[[paste0("Cal",k)]], split = "__")[[1]][4],"_"),
                                             x = paste0("_",Models,"_"))]
            ...
            CalSet <- data.frame(
                ...
                name.gas           = Config$all[["sens2ref"]]$name.gas[match(x = name.sensor, table = Config$all[["sens2ref"]]$name.sensor)],
                name.reference     = Config$all[["sens2ref"]]$gas.reference2use[i.sensors()][k],
                # Setting parameters
                nameGasRef         = paste0("Out.",Config$all[["sens2ref"]]$gas.reference2use[i.sensors()][k]),  # reference gas
                ...
                Cal                = paste0(Config$all[["Server"]]$AirsensEur.name,"__",name.sensor,"__",input[[paste0("Cal",k)]]),# Selected calibration model  for the current sensor
        ...
        observeEvent({
            ...
            if (input[[paste0("Cal.Line",k)]] == "Calibration with current data") {

            } else if (input[[paste0("Cal.Line",k)]] == "Prediction with previous calibration") {

                shinyjs::hide(id = paste0("Slope",k))
                shinyjs::hide(id = paste0("Intercept",k))
                shinyjs::show(id = paste0("Calibration",k) )
                shinyjs::show(id = paste0("Cal",k) )
            ...
            } else if (input[[paste0("Cal.Line",k)]] == "Calibration with slope and intercept") {

                shinyjs::show(id = paste0("Slope",k))
                shinyjs::show(id = paste0("Intercept",k))
                shinyjs::hide(id = paste0("Calibration",k) )
                shinyjs::hide(id = paste0("Cal",k) )
                shinyjs::hide(id = paste0("CovMod",k) )
            }
        }, priority = 800)
        ...
        observeEvent({
            ...
            Splitted.Cal <- unlist(strsplit(x = paste0(Config$all[["Server"]]$AirsensEur.name,"__",CalSet()$name.sensor,"__",input[[paste0("Cal",CalSet()$k)]]), split = "__"))
        ...
        observeEvent({sapply(seq_along(list.name.sensors()), function(i) input[[paste0("DateCOVCal",i)]])
        }, {
            #if (input$DateCALCovCal1>0) {
        ...
        output$ts_Cal_dygraphs <- renderDygraph({
            ...
            if (all(is.na(General.df[, .SD, .SDcols = c(CalSet()$nameGasRef,CalSet()$nameGasMod)]))) {
                ...
                plot_cal <- dygraph(ts_cal,
                                    main = paste0(CalSet()$AirsensEur.name, ": Calibrated ", CalSet()$name.sensor," from ",format(CalSet()$Cal.DateIN,"%d-%b-%y")," to ", format(CalSet()$Cal.DateEND,"%d-%b-%y")
        ...
        Plot.ResCalMatrix    <- reactive({
            ...
            #   DF$General
            #   INFLUX()[[4]]
            #   CalSet()$Cal.DateIN & CalSet()$Cal.DateEND
        ...
        if (input$UserMinsAvg == input$UserMins) {
            ...
            if (!is.null(DF$General)) {
                ...
                Cols.for.Avg <- c(intersect(c("date","gpsTimestamp","altitude","latitude","longitude",
                                              ...
                                              paste0("Out.",list.gas.sensors()), paste0("Out.",list.gas.sensors())),
                                            names(DF$General)),
                                  grep(pattern = paste(c("_volt", "_modelled"), collapse = "|"), x = names(DF$General), value = TRUE))
                set(DF$General, j = Cols.for.Avg, value = lapply(DF$General[,..Cols.for.Avg], nan.to.na))
        ...
        observeEvent({
            ...
            if (input$UserMinsAvg == input$UserMins) {
                ...
                #DF$General[]      <- lapply(DF$General, nan.to.na)
                Cols.for.Avg <- names(DF$General)[-which(names(DF$General) == "date")]
                set(DF$General, j = Cols.for.Avg, value = lapply(DF$General[,..Cols.for.Avg], nan.to.na))
                ...
                Cols.for.Avg <- c(intersect(c("date","gpsTimestamp","altitude","latitude","longitude",
                                  ...
                                  paste0("Out.",list.gas.sensors()), paste0("Out.", list.gas.reference2use()),
                                  grep(pattern = paste(c("_volt", "_modelled"), collapse = "|"), x = names(DF$General), value = TRUE))
                ...
                if (is.null(y)) Cal$Forced <<- TRUE else {
                    ...
                    EtalLim <- Etalonnage( x = x,
                                           ...
                                           Title = paste0(CalSet()$AirsensEur.name, ": ",": Predicted ", CalSet()$name.sensor," data from ",
                                                          format(CalSet()$Meas.DateIN,"%y-%m-%d")," to ",format(CalSet()$Meas.DateEND,"%y-%m-%d")), # , " at ",CalSet()$Reference.name
                                           Marker = 1,
                                           Couleur = "blue",
                                           ligne = 'p',
                                           XY_same = TRUE,
        ...
        output$ts_Extra_dygraphs <- renderDygraph({
            ...
            if (all(is.na(General.df[, .SD, .SDcols = c(CalSet()$nameGasRef,CalSet()$nameGasMod)]))) {
                ...
                plot_extra <- dygraph(ts_extra,
                                      main = paste0(CalSet()$AirsensEur.name, ": Predicted data of ", CalSet()$name.sensor," from ",format(CalSet()$Meas.DateIN,"%d-%b-%y")," to ", format(CalSet()$Meas.DateEND,"%d-%b-%y")
                                                    , " with model ",isolate(CalSet()$NewCalSet)," at ",CalSet()$Reference.name) ) %>%
                    dySeries(CalSet()$nameGasRef,label = Name.pol[1], color = "red") %>%
                    dySeries(CalSet()$nameGasMod,label = Name.pol[2], color = "blue") %>%
                    dyAxis("y", label = Name.pol[2]) %>%
        ...
        Plot.Uncertainty    <- reactive({
            ...
            #----------------------------------------------------------CR
            # depends:
            #   input$ubsRM1, input$ubsRM2, input$ubsRM3, input$ubsRM4
            #   DF.aggregated$Avg
            #   input$Sensors,
            #   Config$all[["sens2ref"]]$name.sensor
            #   DF$General, DF.aggregated$Avg
            #   input[[paste0("DateMeas",k)]]
            #   input[[paste0("DatePlotMeas",k)]]
            # isolates:

            # depends on changes of input$ubsRM, input$ubss and DF.aggregated$Avg
            input$ubsRM1
            input$ubsRM2
            input$ubsRM3
            input$ubsRM4
            ...
            if (all(is.na(General.df[, .SD, .SDcols = c(CalSet()$nameGasRef,CalSet()$nameGasMod)]))) {
                ...
                calib <- slope_orth(
                    ...
                    Units    = CalSet()$unit.sensor,
                    Dir      = CalSet()$WDModelled_gas,
                    Mat      = cbind(1:nrow(General.df), General.df[,.SD, .SDcols = c("date", CalSet()$nameGasRef,CalSet()$nameGasMod)]),
                    ubsRM    = as.numeric(CalSet()$ubsRM),
                    ubss     = as.numeric(CalSet()$ubss),
                    variable.ubsRM = FALSE,
                    variable.ubss  = FALSE,
            ...
            return(
                list(data.frame(values = sapply(calib[which(names(calib) %in% names(calib)[-which(names(calib) %in% c("Mat", "RS.Fitted"))])],function(x) x),
                                # units of parameters= # c("mo","sdo", "mm","sdm", "b1", "ub1", "b0", "ub0", "RSS","rmse", "mbe", "Correlation", "nb", "CRMSE", "NMSD", "Mat", "RS.Fitted")
                                units  = c(paste0(CalSet()$unit.sensor),
                                           paste0(CalSet()$unit.sensor),
                                           paste0(CalSet()$unit.sensor),
        ...
        plot.Target    <- reactive({
            ...
            #
            #     dev.off
            #     # svg(filename = u.Target.File,
            #     #     width = 7, height = 7, pointsize = 12,
            #     #     onefile = TRUE, family = "sans", bg = "white")
            #     cat(paste0("[shiny] INFO, ", u.Target.File," saved.\n" ))
            #
            #     updateCheckboxInput(session, inputId = "SavePlot", label = NULL, value = FALSE)
            # }

            # Saving the plot of orthogonal regression
            #dev.copy(svg,filename = u.Target.File, width = 16, height = 14, pointsize = 12, onefile = TRUE, family = "sans", bg = "white");
            dev.off()
        ...
        #
        #     # Generate plot every time the function is called
        #     #----------------------------------------------------------CR
        #     # Plotting Target diagram
        #     #----------------------------------------------------------CR
        #     # depends:
